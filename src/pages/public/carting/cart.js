                  ...
                  state.cart.cart
                    .map(i => {
                      ...
                      if (product && product.discount) {
                        ...
                        const activeDiscounts = discounts.filter(d => d.discount_active === 1);

                        console.log("here: ", activeDiscounts);
                        if (activeDiscounts.length > 0) {
                          activeDiscounts.sort((a, b) => b.discount_active - a.discount_active);
                          const discountPercentage = parseInt(activeDiscounts[0].discount_percentage);
                          const discountedPrice = (1 - discountPercentage / 100) * product.price * i.amount;
                          return discountedPrice;
                        }
                      }
                      return product.price * i.amount;
                    })
                    .reduce((a, b) => a + b, 0)
                )
              }
                  ...
                  formatPrice(
                    state.cart.cart
                      .map((cartItem) => {
                        ...
                        if (product && product.discount && Array.isArray(product.discount) && product.discount.length > 0) {
                          const activeDiscount = product.discount.find(
                            (discount) => discount.discount_active === 1
                          );
                          if (activeDiscount) {
                            const discountPercentage = parseInt(activeDiscount.discount_percentage);
                            const discountedPrice = (1 - discountPercentage / 100) * product.price * cartItem.amount;
                            return discountedPrice;
                          }
                        }
