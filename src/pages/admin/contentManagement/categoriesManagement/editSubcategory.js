...
import {ColoredTextInput, FileUploader} from "../../../../components/forms";
...
import {setSubcategory, updateCategories, updateSubcategoryProp__L1} from "../../../../store/reducers/categoryReducer";
import ReactQuill from "react-quill";

export const EditSubcategory = () => {
    const {product_category_id, id} = useParams()
    ...
    useEffect(() => {
        dispatch(setSubcategory({cat: parseInt(product_category_id), subCat: parseInt(id)}))
    }, [])
    ...
    return (
        <div
            ...
            <div className="w-fit bg-white py-8 px-12"
                ...
                <div
                    className="overflow-y-scroll border-b scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-slate-300">
                ...
                <div className="w-full mt-4">
                    <ButtonGroup className={"justify-end"}>
                        <LinkButton location={"/admin/content/categories"} size={"sm"} text={"Close"} bg={"red"}
                                    color={"white"}/>
                        <Button onClick={() => updateSubcategory()} size={"sm"} text={"Save"} bg={"blue"}
                                color={"white"}/>
                    </ButtonGroup>
                </div>
            </div>
